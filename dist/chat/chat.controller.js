...
let ChatController = class ChatController {
    ...
    async sendMassage(msgobj) {
        return this.chatService.sendMassage(msgobj);
    }
    async findMessages(body) {
        const { sender, receiver } = body;
        const messages = await this.chatService.findMessagesBySenderAndReceiver(sender, receiver);
        return messages;
    }
    async list(body) {
        const { sender, receiver } = body;
...
], ChatController.prototype, "sendMassage", null);
__decorate([
